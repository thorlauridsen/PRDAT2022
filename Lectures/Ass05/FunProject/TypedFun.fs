...
let rec typ (e : tyexpr) (env : typ env) : typ =
    ...
    | ListExp(lst, listType) -> 
      let invalidTypes = List.fold (fun acc elem -> 
                                       if ((typ elem env) <> listType) then 
                                           acc + 1 
                                       else 
                                           acc
                                   ) 0 lst 
      if invalidTypes = 0 then 
          TypL listType
      else 
          failwith "ListExp: list contains invalid types"

let typeCheck e = typ e [];;
let ex1 = ListExp ([CstI 5; CstI 4; CstI 3], TypI);;
let ex2 = ListExp ([CstI 5; CstB false; CstI 3], TypI);;
